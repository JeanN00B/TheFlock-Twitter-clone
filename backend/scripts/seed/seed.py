...
LIKES_DEFERRED_NOTE = "Likes are deferred: like endpoints exist but the likes phase ships separately."
...
PHASES = ("users", "tweets", "follows", "all")
...
def run_phase(phase: str, base_url: str, seed_key: str, fixture: dict) -> dict:
    ...
    if phase == "all":
        ...
        return {"users": users, "tweets": tweets, "follows": follows}
...
def print_summary(phase: str, result: dict, usernames: list[str]) -> None:
    if phase == "all":
        ...
        per_user = result["tweets"]["per_user"]
    elif phase == "follows":
        graph = result["graph"]
        per_user = {}
    else:
    ...
    for targets in graph.values():
        for target in targets:
            followers[target] += 1
    if graph or per_user:
        print("\nusername   tweets  following  followers")
        for name in usernames:
            print(
                f"{name:<10} {per_user.get(name, '-'):>6}  "
